...
import { TagShow } from "~/common/exam/tag";
import { ExamQuestion } from "~/common/exam/question";
...
function ExamPaperMeta({ paperMeta }: ExamPaperMetaProps) {
  ...
  const getGroupName = (index: number, typeName: string, subTitle: string) => {
    return (
      <div className="text-base">
        ...
        {StringValidator.isNonEmpty(subTitle) && <div>({subTitle})</div>}
      </div>
    );
  };
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* 标题 */}
      <div className="text-2xl text-center">{paperMeta.title}</div>
