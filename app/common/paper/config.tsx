...
interface PaperMetaSearchConfProps {
  textbooks: Textbook[];
  metaSearch: PaperMetaSearch;
  updateSearchMeta: (key: keyof PaperMetaSearch, value: string | number | string[]) => void;
}

function PaperMetaSearchConf({ textbooks, metaSearch, updateSearchMeta }: PaperMetaSearchConfProps) {
  return (
    <>
      ...
      {metaSearch.source !== "list" && (
        <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-4">
          ...
          <div className="flex-1 min-w-0">
            <StatusSelect defaultValue={metaSearch.status} onSelect={(status) => updateSearchMeta("status", status)} />
          </div>
        </div>
      )}
    </>
...
export { PaperMetaSearchConf, PaperMetaConf };
