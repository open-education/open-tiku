...
export default function Add(props: any) {
  ...
  const [sourceVal, setSourceVal] = useState<string>(
    StringConstUtil.tikuSourceList[0].value,
  );
  ...
  const onSelectQuestionTagChange: GetProp<
    typeof Checkbox.Group,
    "onChange"
  > = (checkedValues) => {
    setTagsVal(checkedValues.map(Number).filter((val) => !isNaN(val)));
  };
        ...
        >
          <Form.Item label="题目来源">
            <Select
              ...
              options={StringConstUtil.tikuSourceList}
            />
          </Form.Item>
