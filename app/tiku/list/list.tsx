...
export function ListInfo(props: any) {
    ...
    const getTextbookKey = () => {
        if (StringValidator.endsWith(pathname, StringConst.chapter)) {
            return StringUtil.getFirstPart(pathname, StringConst.chapter);
        }
        if (StringValidator.endsWith(pathname, StringConst.knowledge)) {
            const knowledgeInfoKey = leftMenuItemSelectKeyPath[leftMenuItemSelectKeyPath.length - 1];
            return StringUtil.removeLastPart(knowledgeInfoKey);
        }
        return "";
    }
    const textbookKey = getTextbookKey();
    ...
    useEffect(() => {
        if (leftMenuItemSelectKey.length == 0 || textbookKey.length == 0) {
            return;
        }
    ...
    }, [leftMenuItemSelectKey, refreshListNum]);
    ...
    return <div>
        ...
        <Row gutter={[15, 15]}>
            <Col span={24}>
                {/* 面包屑快速导航 */}
                {CommonBreadcrumb(subjectList, catalogList, knowledgeInfoList, pathname, leftMenuItemSelectKey)}
            </Col>
        ...
        {/* 一个选择题的样式 */}
        {
            questionListResp.data?.map(questionInfo => {
                ...
                >
                    ...
                    {/* 题目其它标签, 比如查看答案, 关联题目等 */}
                    {CommonQuickJumpTag(questionInfo, setOpenDrawer, setDrawerTitle, setDrawerContent, setRefreshListNum, questionTypeList, tagList, catalogList, knowledgeInfoList)}
                </div>
            })
        }
