...
export function EditOptions(props: EditOptionProps) {
  ...
  const updateOptions = () => {
    ...
    httpClient.post<boolean>("/edit/options", req).then((res) => {
      setShowEditOptionsErr("");
      setShowEditOptions(false);
      props.setRefreshListNum(StringUtil.getRandomInt());
    }).catch((err) => {
      setShowEditOptionsErr(<div>
        <Alert title={`更新选项出错: ${err.message}`} type={"error"}/>
      </div>);
    });
  }

  const showEditOptionsArea = <div className="mt-2.5">
    <Flex gap="small" wrap justify={"right"}>
      <Button color="cyan" variant="dashed" onClick={updateOptions}>更新</Button>
    </Flex>
  </div>
