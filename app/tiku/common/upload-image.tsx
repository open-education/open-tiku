...
export function EditUploadImageStyle(props: EditUploadImageProps) {
  ...
  const updateImageVal = () => {
    ...
    httpClient.post<boolean>("/edit/images", req).then(res => {
      setShowEditImageErr("");
      setShowEditImage(false);
      props.setRefreshListNum(StringUtil.getRandomInt());
    }).catch(err => {
      setShowEditImageErr(<div>
        <Alert title={`更新图片出错: ${err.message}`} type={"error"}/>
      </div>);
    });
  }

  const showEditImageArea = <div className="mt-2.5">
    <Flex gap="small" wrap justify={"right"}>
      <Button color="cyan" variant="dashed" onClick={updateImageVal}>更新</Button>
    </Flex>
  </div>
